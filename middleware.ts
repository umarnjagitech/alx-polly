...
export async function middleware(request: NextRequest) {
  ...
  // If env vars are missing, skip auth in middleware to avoid network errors
  if (!url || !anonKey) {
    return response
  }
  ...
  try {
    ...
    // Ignore middleware auth fetch failures; app-level guards will handle auth
  }
...
export const config = {
  matcher: [
     ...
     */
    '/((?!_next/static|_next/image|favicon.ico).*?trpc.*|api|/|polls|profile|auth/login|auth/register|$)',
  ],
}
